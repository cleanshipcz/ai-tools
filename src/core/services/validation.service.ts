...
interface ManifestFile {
  path: string;
  type: 'prompt' | 'agent' | 'ruleset' | 'skill' | 'eval' | 'project' | 'recipe' | 'feature' | 'deploy';
  id: string;
  version?: string;
  content: any;
}
...
export class ValidationService {
  ...
  private async collectManifests(result: ValidationResult): Promise<ManifestFile[]> {
    ...
    const dirs = {
      ...
      [this.config.dirs.rulesets]: 'ruleset',
  ...
  private validateReferences(manifests: ManifestFile[], result: ValidationResult): void {
    for (const manifest of manifests) {
      ...
      if (manifest.content.rulesets) {
        for (const rulesetId of manifest.content.rulesets) {
          if (!this.manifestsById.has(rulesetId)) {
            result.errors.push(
              `${relative(this.config.rootDir, manifest.path)}: Referenced ruleset "${rulesetId}" not found`
            );
          }
        }
      }
      ...
      if (manifest.content.extends) {
        for (const parentId of manifest.content.extends) {
          if (!this.manifestsById.has(parentId)) {
            result.errors.push(
              `${relative(this.config.rootDir, manifest.path)}: Extended ruleset "${parentId}" not found`
            );
          }
        }
      }
