...
describe('ResolverService pattern-based filters', () => {
  ...
  it('should include/exclude rulepacks using allow/deny patterns', async () => {
    vi.spyOn(resolver as any, 'loadRulepack').mockImplementation(async () => ({ rules: [], tags: [] }));
    ...
    const project: Project = {
      ...
      name: 'Test',
      description: 'desc',
      rulepacks: { include: ['typescript'] },
    };

    await expect(resolver.shouldIncludeRulepack('coding-typescript', project)).resolves.toBe(true);
    await expect(resolver.shouldIncludeRulepack('coding-python', project)).resolves.toBe(false);

    project.rulepacks!.exclude = ['typescript'];
    await expect(resolver.shouldIncludeRulepack('coding-typescript', project)).resolves.toBe(false);
  });
