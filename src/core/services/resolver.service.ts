...
import { Ruleset, Project, AIModel, Agent, Prompt } from '../models/types.js';
...
export class ResolverService {
  ...
  /**
   * Resolve rulesets into a flat list of rules, handling inheritance (extends).
   * Optionally filters rulesets based on project configuration (regex allow/deny patterns and tech-stack).
   */
  async resolveRulesets(rulesetIds: string[], project?: Project, techStackContext?: { languages?: string[] }): Promise<string[]> {
    ...
    const resolve = async (id: string) => {
      ...
      // Check if we should include this ruleset based on project config
      if (project && !(await this.shouldIncludeRuleset(id, project, techStackContext))) {
        return;
      }

      const ruleset = await this.loadRuleset(id);
      if (!ruleset) {
        console.warn(`Warning: Ruleset "${id}" not found`);
        return;
      }

      // Resolve parent rulesets first
      if (ruleset.extends) {
        for (const parentId of ruleset.extends) {
          await resolve(parentId);
        }
      }

      // Add this ruleset's rules
      resolved.push(...ruleset.rules);
    };
  ...
  async loadRuleset(rulesetId: string): Promise<Ruleset | null> {
    try {
      const rulesetPath = this.config.getPath(this.config.dirs.rulesets, `${rulesetId}.yml`);
      return await this.loader.loadYaml<Ruleset>(rulesetPath, false);
    } catch {
      return null;
    }
  }

  async shouldIncludeRuleset(rulesetId: string, project: Project, techStackContext?: { languages?: string[] }): Promise<boolean> {
    if (!this.passesFilters(rulesetId, project.rulesets)) return false;
    ...
    const ruleset = await this.loadRuleset(rulesetId);
    // Check tags at root level (as per YAMLs) or in metadata (legacy/fallback)
    const tags = ruleset?.tags || ruleset?.metadata?.tags;

    if (tags) {
      const rulesetTags = tags.map((tag) => tag.toLowerCase());

      if (languages && languages.length > 0) {
        // We have a specific language context
        const techStackLanguages = languages.map((lang) => lang.toLowerCase());
        const hasLanguageMatch = rulesetTags.some((tag) => techStackLanguages.includes(tag));

        // Check if ruleset is language-specific
        ...
        const isLanguageSpecific = rulesetTags.some((tag) => 
          [
        ...
        // If the ruleset is tagged with ANY language that is used in the project's tech stacks, exclude it.
        // This prevents "python" rules from leaking into the global agent when "python" is only in the "backend" stack.
        const allProjectLanguages = this.getAllProjectLanguages(project);
        const isSpecificToSomeStack = rulesetTags.some(tag => allProjectLanguages.includes(tag));
  ...
  async resolveAllAgents(project: Project): Promise<{ agent: Agent; rules: string[]; suffix: string }[]> {
    ...
    const processContext = async (suffix: string, context?: { languages?: string[] }) => {
      for (const file of agentFiles) {
        ...
        if (this.shouldIncludeAgent(agent.id, project)) {
          const rules = agent.rulesets ? await this.resolveRulesets(agent.rulesets, project, context) : [];
    ...
    // Our updated shouldIncludeRuleset will ensure they don't have language-specific pollution.
