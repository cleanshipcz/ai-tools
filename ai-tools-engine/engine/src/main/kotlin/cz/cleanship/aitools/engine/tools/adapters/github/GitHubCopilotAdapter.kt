...
class GitHubCopilotAdapter(
    ...
    override fun prepare(projectDir: File, project: ProjectManifest) {
        // Why these three directories are wiped at all: every file in them is generated by this adapter and
        // rewritten on each export, so removing them can never take anything but our own previous output, and
        // without the wipe files from retired naming schemes linger and keep polluting the VS Code prompt picker.
        // The wipe is nevertheless gated on the project-global `deploy.replace` flag, exactly like every other
        // adapter, so that one flag decides for all tools whether a deploy may delete. The accepted consequence:
        // with `replace: false` output of a retired naming scheme survives here and has to be removed by hand -
        // that is the trade-off of a single consistent flag, not an oversight, so do not make it unconditional
        // again. `.github` itself is deliberately left alone because it also holds `workflows/`, and
        // `copilot-instructions.md` is overwritten in place.
