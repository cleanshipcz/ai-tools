...
- `rulepacks/` → `01_rulepacks/` - Coding rules and standards (foundation for everything)
- `skills/` → `02_skills/` - Executable commands and MCP integrations
- `prompts/` → `03_prompts/` - Prompt manifests (uses rulepacks)
- `agents/` → `04_agents/` - Agent manifests (uses prompts + rulepacks)

### Workflows & Projects (05-07)
- `recipes/` → `05_recipes/` - Multi-agent workflow definitions (uses agents)
- `projects/` → `06_projects/` - Project configurations (global/local)
- `mcp/` → `07_mcp/` - MCP server configurations and presets

### Build System & Validation (10-12)
- `schemas/` → `10_schemas/` - JSON Schema definitions
- `scripts/` → `11_scripts/` - Build, validation, and utility scripts
- `templates/` → `12_templates/` - Templates for adapter generation

### Configuration (15)
- `config/` → `15_config/` - Runtime configuration (budgets, providers, deploy)

### Testing & Quality Assurance (20-21)
- `evals/` → `20_evals/` - Evaluation framework and datasets
- `redteam/` → `21_redteam/` - Security testing (jailbreaks, unsafe cases)

### Documentation & Learning (90-91)
- `docs/` → `90_docs/` - Documentation files
- `examples/` → `91_examples/` - Example usage and tutorials
...
### Grouping Strategy
1. **01-04**: Foundation layer in logical dependency order (rulepacks → skills → prompts → agents)
...
2. ☐ Review all file references in scripts
...
#### Configuration Files
- `package.json` - Update script paths (scripts/ → 11_scripts/)
- `tsconfig.json` - Update include paths (scripts/ → 11_scripts/)
...
- ✅ `adapters/` - **NO CHANGE** - Generated, gitignored, rarely referenced
- ✅ `config/` → `15_config/` - Only referenced in eval.ts
- ✅ `examples/` → `91_examples/` - Standalone, few references
- ✅ `redteam/` → `21_redteam/` - Standalone, documentation only

### Medium Impact (Several Script Updates)
- ⚠️ `docs/` → `90_docs/` - Many cross-references in README.md
- ⚠️ `evals/` → `20_evals/` - Referenced in eval.ts and clean.ts
- ⚠️ `recipes/` → `05_recipes/` - Referenced in run-recipe.ts
- ⚠️ `mcp/` → `07_mcp/` - Referenced in documentation
- ⚠️ `templates/` → `12_templates/` - Referenced in gen-project.ts

### High Impact (Many References Throughout)
- 🔴 `prompts/` → `03_prompts/` - Referenced in multiple scripts, docs, templates
- 🔴 `agents/` → `04_agents/` - Referenced in build, docs, templates
- 🔴 `rulepacks/` → `01_rulepacks/` - Referenced in build, validation
- 🔴 `skills/` → `02_skills/` - Referenced in build, gen-skills
- 🔴 `projects/` → `06_projects/` - Referenced in many project-related scripts
- 🔴 `schemas/` → `10_schemas/` - Referenced in validation, potentially imports
- 🔴 `scripts/` → `11_scripts/` - Self-referential in package.json
...
### ✅ DECIDED: Do NOT rename `adapters/`
...
### ✅ DECIDED: Use underscore format `XY_`
